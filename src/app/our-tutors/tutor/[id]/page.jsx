...
export default function TutorDetailPage() {
  ...
  const fetchTutorDetail = async () => {
    try {
      ...
      const response = await axios.get(
        `${API_URL}/api/courses/teachers/${tutorId}`,
      );
  ...
  useEffect(() => {
    ...
    try {
      const raw = sessionStorage.getItem('selectedTutor');
      if (raw) {
    ...
    const fromMock = mockTutors.find((t) => String(t.id) === String(tutorId));
    if (fromMock) {
      setTutor(fromMock);
      setLoading(false);
      return;
    ...
    return (
      <div className="min-h-screen bg-white flex items-center justify-center">
        <div className="text-center">
          <h2 className="text-2xl md:text-3xl font-bold text-[#313D6A] mb-2">
            Tutor not found
          </h2>
                ...
                <CardContent className="p-5">
                  <div className="text-center mb-6">
                    ...
                    <h2 className="text-xl md:text-2xl lg:text-3xl font-bold mb-1">
                      Tutor ID: PT{tutor?.id}
                    </h2>
                    <p className="text-sm md:text-base text-gray-300 mb-3">
                      Member Since {tutor?.member_since ?? 'N/A'}
                    </p>
                  </div>
                  ...
                  <div className="space-y-2 mb-6 text-sm md:text-base">
                    ...
                    <div className="flex justify-between">
                      <span className="text-[#F5BB07] font-semibold">
                        City:
                      </span>
                  ...
                  <div className="flex justify-center">
                    <Button className="bg-[#F5BB07] hover:bg-[#F5BB07]/90 text-[#313D6A] text-sm md:text-base">
                      Book a session
                    </Button>
                  </div>
                </CardContent>
                  ...
                  <div className="overflow-x-auto">
                    ...
                    <table className="min-w-full table-fixed border border-gray-300 border-collapse text-sm md:text-base">
                      <thead className="bg-white">
                        <tr>
                          <th className="px-3 py-2 text-left text-xs md:text-sm font-semibold text-gray-700 uppercase tracking-wider border border-gray-300 w-1/4">
                            Degree
                          </th>
                          <th className="px-3 py-2 text-left text-xs md:text-sm font-semibold text-gray-700 uppercase tracking-wider border border-gray-300 w-1/4">
                            Subject
                          </th>
                          <th className="px-3 py-2 text-left text-xs md:text-sm font-semibold text-gray-700 uppercase tracking-wider border border-gray-300 w-1/6">
                            Passing Year
                          </th>
                          <th className="px-3 py-2 text-left text-xs md:text-sm font-semibold text-gray-700 uppercase tracking-wider border border-gray-300 w-1/3">
                            Institute
                          </th>
                        </tr>
                      </thead>
                ...
                <CardContent className="p-4">
                  <div className="overflow-x-auto">
                    <table className="min-w-full table-fixed border border-gray-300 border-collapse text-sm md:text-base">
                      <thead className="bg-white">
                        <tr>
                          <th className="px-3 py-2 text-left text-xs md:text-sm font-semibold text-gray-700 uppercase tracking-wider border border-gray-300">
                            Position
                          </th>
                          <th className="px-3 py-2 text-left text-xs md:text-sm font-semibold text-gray-700 uppercase tracking-wider border border-gray-300">
                            From
                      ...
                      <tbody className="bg-white divide-y divide-gray-100">
                        {(tutor?.experience ?? []).length > 0 ? (
                          ...
                          <tr>
                            <td
                              colSpan={5}
                              className="px-3 py-4 text-center text-gray-500 border border-gray-300"
                            >
              ...
              <Card className="bg-[#FFFCE0] rounded-none lg:rounded-l-2xl lg:rounded-r-none overflow-hidden w-full border">
                ...
                <CardContent className="p-4 space-y-4 text-sm md:text-base">
                  ...
                  <div className="space-y-2">
                    <div className="bg-[#313D6A] inline-block text-white px-3 py-2 rounded-full text-sm font-semibold">
                      Days Availability
                    </div>
                    <div className="p-3 border border-gray-200 space-y-1">
                      {getAvailabilityDays(tutor?.availability_schedule).map(
                        (slot, index) => (
                          <div
                            key={index}
                  ...
                  <div className="space-y-2">
                    ...
                    <div className="p-3 rounded-b ">
                      <div className="flex items-center text-sm md:text-base">
                        ...
                        <span>
                          {tutor?.time_availability ?? 'Morning, Evening'}
                        </span>
                      </div>
                    </div>
                  </div>
                  ...
                  <div className="space-y-2">
                    ...
                    <div className="p-3 rounded-b ">
                      <div className="flex items-center text-sm md:text-base">
                        ...
                        <span>
                          {(tutor?.expertise_areas || []).join(', ') || 'N/A'}
                        </span>
                      </div>
                    </div>
                  </div>
                      ...
                      <div className="text-sm md:text-base">
                        <div
                          className={`font-semibold ${
                            tutor?.online ? 'text-green-600' : 'text-gray-600'
                          }`}
                        >
                          {tutor?.online ? 'Yes' : 'No'}
                        </div>
                  ...
                  <div className="space-y-2">
                    ...
                    <div className="p-3 rounded-b space-y-1 text-sm md:text-base">
                      <div className="text-sm md:text-base">
                        {(tutor?.languages_spoken ?? []).join(' & ') || 'N/A'}
                      </div>
                    </div>
                  </div>
                  ...
                  <div className="space-y-2">
                    ...
                    <div className="p-3 rounded-b space-y-1 text-sm md:text-base">
                      <div className="text-sm md:text-base font-semibold text-[#313D6A]">
                        Charge/Hour: {tutor?.hourly_rate ?? 'N/A'}
                      </div>
                    </div>
                  </div>
                          ...
                          >
                            ...
                            <div className="flex">
                              {[...Array(5)].map((_, i) => (
                                <Star
                                  key={i}
                                  className="h-3 w-3 text-[#F5BB07] fill-current"
                                />
                              ))}
                            </div>
                          </div>
