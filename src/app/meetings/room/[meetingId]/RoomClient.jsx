...
export default function MeetingRoom({ meetingId, participantName, isHost = false, signalingUrl }) {
    ...
    const resolvedSignalingUrl = signalingUrl || `ws://localhost:8000/ws/meeting/${roomId}/`
    ...
    useEffect(() => {
        ...
        ws.onmessage = (event) => {
            ...
            switch (msg.type) {
                case "offer":
                    console.log("➡️ Got offer from peer")
                    break
                case "answer":
                    console.log("➡️ Got answer from peer")
                    break
                case "candidate":
                    console.log("➡️ Got ICE candidate")
                    break
                case "joined":
                    console.log("🙋 Successfully joined meeting:", msg)
                    break
                default:
    ...
    }, [signalingUrl, meetingId, participantName, isHost])

    useEffect(() => {
        const ws = new WebSocket(resolvedSignalingUrl)
        wsRef.current = ws

        ws.onopen = () => {
            console.log("✅ Connected to signaling server:", resolvedSignalingUrl)
            setConnected(true)
        }
